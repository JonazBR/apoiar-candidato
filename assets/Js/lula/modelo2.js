...
const modelo2 = async (name) => {
    return new Promise(async(resolve) => {
        ...
        const template = await canvas.loadImage("./assets/Images/lula/2.png")
        ...
        if(name.length > 6){
            values.x = values.x - name.length * 25 // mudar a posição do nome
        } 
        if(name.length > 8) {
            values.letter = values.letter - name.length - 30 // mudar tamanho da letra
        }
